...
function columnDefsfn(colHd, sample) {
    ...
    var columndef = []
    colHd.forEach(ele => columndef.push({
        headerName: ele,
        field: ele
    }));
...
function subSheet() {
    ...
    var yourNewSheet = activeSpreadsheet.getSheetByName("subGrid");

    if (yourNewSheet != null) {
        activeSpreadsheet.deleteSheet("subGrid");
    }

    SpreadsheetApp.getActiveSpreadsheet().insertSheet("subGrid");
}
